...
**结论**：PinWall 的交互基线应该对标 CleanShot X。剪贴板优先 + 悬浮托盘这两点是体验分水岭。
...
**结论**："最短路径" 的思路必须保留 —— 无论 PinWall 后面堆多少功能，默认路径都要保持这么快。
...
**值得借鉴的是历史库的呈现方式**：PinWall 的截图历史如果做成卡片时间线而非文件列表，检索效率会高一个量级。

## 对 PinWall 的启示（已根据补充调研修订）

1. ~~跨平台一致性是最大的空白~~ —— **此结论有误，已作废**。Snipaste 已经覆盖 Windows / macOS / Linux 三平台。平台覆盖不是差异化点。
2. **贴图必须是一等公民**：这是前五名集体缺失、而 Snipaste 用户最离不开的能力。PinWall 若没有贴图，对 Snipaste 用户就没有迁移理由。
